...
_Y_METRIC = "dist_worst_ratio"
_Y_LABEL  = "Worst / mean ratio"


def plot_pareto_worst_mean(
    df: pd.DataFrame,
    output_path: Path,
    *,
    ...
    """Scatter total_distance vs dist_worst_ratio per instance. output_path is a directory."""
...
def plot_pareto_worst_mean_aggregated(
    df: pd.DataFrame,
    output_path: Path,
    *,
    group_col: str = "algorithm",
) -> None:
    """Single PNG: mean rel_distance vs mean worst_ratio, one point per algorithm."""
    output_path = Path(output_path)
    ...
    ax.set_title("Pareto: worst/mean ratio (aggregated)")
    fig.tight_layout()
    save_fig(fig, output_path)


def plot_pareto_worst_mean_single(result, output_path: Path) -> None:
    ...
    _plot_pareto(
        ...
        [result.fairness.distance.worst_ratio],
        [algo],
        colors,
        y_label=_Y_LABEL,
    )
